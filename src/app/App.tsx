...
export default function App() {
  ...
  const allTags = useMemo(() => Array.from(new Set(components.flatMap(c => c.meta.tags))).sort(), [])
  const allTech = useMemo(() => Array.from(new Set(components.flatMap(c => c.meta.tech))).sort(), [])
    ...
    if (activeTags.length > 0) {
      result = result.filter(reg => 
        activeTags.every(active => 
          reg.meta.tags.some(t => t.toLowerCase() === active.toLowerCase()) ||
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#020202] selection:bg-accent-500/30">
      ...
      <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
        ...
        <div 
          className="absolute top-[5%] right-[5%] w-[600px] h-[600px] rounded-full animate-blob-pulse opacity-40"
          style={{
            background: 'radial-gradient(circle, #e85002 0%, rgba(232, 80, 2, 0) 70%)',
            filter: 'blur(100px)',
            mixBlendMode: 'screen'
          }}
        />
        ...
        <div 
          className="absolute top-[30%] right-[-10%] w-[800px] h-[800px] rounded-full animate-blob-pulse-delayed opacity-30"
          style={{
            background: 'radial-gradient(circle, #c10801 0%, rgba(193, 8, 1, 0) 70%)',
            filter: 'blur(120px)',
            mixBlendMode: 'screen'
          }}
        />
        ...
        <div 
          className="absolute bottom-[-10%] left-[-10%] w-[500px] h-[500px] rounded-full animate-blob-pulse opacity-20"
          style={{
      ...
      <div className="relative z-10 flex w-full h-full">
        ...
        <main className="flex-1 flex flex-col h-full bg-surface-950/20 backdrop-blur-[2px]">
          <Routes>
            <Route path="/" element={<HomePage query={searchText} filtered={filtered} activeTags={activeTags} />} />
