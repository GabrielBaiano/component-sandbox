  ...
  return (
    <div className="flex items-center justify-center gap-4 p-12">
      <button
        ...
        disabled={disabled}
        className={`relative rounded-full transition-opacity ${s.track} ${disabled ? 'cursor-not-allowed opacity-40' : 'cursor-pointer'}`}
        style={{ background: on ? '#e85002' : 'rgba(255,255,255,0.15)' }}
      >
        <motion.div
          animate={{ x: on ? s.on : 2 }}
          transition={{ type: 'spring', stiffness: 500, damping: 30 }}
          className={`absolute top-0.5 rounded-full bg-white shadow ${s.thumb}`}
        />
      </button>
